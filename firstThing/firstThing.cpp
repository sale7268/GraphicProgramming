...
void render_scene(
    ...
    glDepthMask(GL_TRUE);
    {
        ...
        glm::vec4 position = glm::vec4(-1.0f, 0.0f, -1.0f, 1.0f);
...
int main(void)
{
    ...
    GLuint postProcess = compile_shader("postprocess.vert.txt", "postprocess.frag.txt");
    ...
    particles.push_back(Particle(
        glm::translate(glm::mat4(1), glm::vec3(3.5f, 0.0f, -5.5f)),
        glm::vec3(0.0f, 0.0f, 0.0f)
        ));

    particles.push_back(Particle(
        glm::translate(glm::mat4(1), glm::vec3(8.5f, -2.33f, 2.5f)),
        glm::vec3(-1.0f, -2.0f, 0.0f)
        ));

    particles.push_back(Particle(
        glm::translate(glm::mat4(1), glm::vec3(0.5f, 0.33f, -4.5f)),
        glm::vec3(-1.0f, -2.0f, 0.0f)
        ));
    ...
    string bugatti = "bugatti-normals.obj";
    string plane = "plane-normals.obj";
    string monkey = "monkey-normals.obj";
    //Model loading
    vector<Model> model;
    model.push_back(Model::load(cube));
    model.push_back(Model::load(plane));
    model.push_back(Model::load(bugatti));
    model.push_back(Model::load(monkey));

    //Loading textures from file:
    textures.push_back(load_texture(GL_TEXTURE0, "Tape001_2K_Color.jpg"));
    textures.push_back(load_texture(GL_TEXTURE1, "Tape001_2K_Roughness.jpg"));
    textures.push_back(load_texture(GL_TEXTURE2, "Tape001_2K_Normal.jpg"));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        {
            ...
            //Diffuse Map
            GLint diffuse_map_location = glGetUniformLocation(postProcess, "tex");
            glUniform1i(diffuse_map_location, 4); //GL_Texture0

            if (x < 1 && check) {
                GLfloat blackness_location = glGetUniformLocation(postProcess, "blackness");
                glUniform1f(blackness_location, x);
                x += 0.0005;
            }
            else {
                check = false;
